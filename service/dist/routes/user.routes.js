 ...
 */
exports.userRoutes.get('/me/bookings/:bookingId', auth_middleware_1.authMiddleware, (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    try {
      ...
      WHERE b.id = ? AND b.customer_id = ?`, [bookingId, userId]);
        ...
        let hotelImages = [];
        if (booking.hotelId) {
            const [images] = yield pool.query(`SELECT image_url as url FROM hotel_images WHERE hotel_id = ? ORDER BY display_order LIMIT 5`, [booking.hotelId]);
            hotelImages = images;
        }
        ...
        if (booking.hotelId) {
            ...
            const [closestGates] = yield pool.query(`SELECT hg.name_english, hg.name_arabic, hg.gate_number, hgd.distance_meters, hgd.walking_time_minutes
         FROM hotel_gate_distances hgd
         JOIN haram_gates hg ON hgd.gate_id = hg.id
         WHERE hgd.hotel_id = ?
         ORDER BY hgd.distance_meters ASC
         LIMIT 1`, [booking.hotelId]);
            ...
            const [kaabaGates] = yield pool.query(`SELECT hg.name_english, hg.name_arabic, hg.gate_number, hgd.distance_meters, hgd.walking_time_minutes
         FROM hotel_gate_distances hgd
         JOIN haram_gates hg ON hgd.gate_id = hg.id
        ...
        ctx.body = {
            booking: {
                ...
                hotel: {
                    ...
                    images: hotelImages.map((img) => img.url),
                },
                // Proximity info
                proximity: {
                    walkingTimeToHaram,
 ...
 */
exports.userRoutes.post('/me/bookings/:bookingId/cancel', auth_middleware_1.authMiddleware, (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = ctx.state.userId;
        const { bookingId } = ctx.params;
        // @ts-ignore
        ...
        const pool = (0, connection_1.getPool)();
        // Fetch the booking
        const [bookings] = yield pool.query(`SELECT b.*, h.cancellation_policy 
       FROM bookings b
       LEFT JOIN hotels h ON b.hotel_id = h.id
       WHERE b.id = ? AND b.customer_id = ?`, [bookingId, userId]);
        if (!bookings || bookings.length === 0) {
        ...
        yield pool.query(`UPDATE bookings 
       SET status = 'CANCELLED', 
           refund_amount = ?,
           refund_reason = ?,
           refunded_at = NOW(),
           updated_at = NOW()
       WHERE id = ?`, [refundAmount, reason || 'Customer requested cancellation', bookingId]);
        ctx.body = {
            success: true,
 ...
 */
exports.userRoutes.get('/me/bookings', auth_middleware_1.authMiddleware, (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = ctx.state.userId;
      ...
      `;
        ...
        const formattedBookings = yield Promise.all(bookings.map((booking) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            if (!hotelName && (booking.hotelId || metadata.hotelId)) {
                try {
                    const [hotelRows] = yield pool.query('SELECT name FROM hotels WHERE id = ? LIMIT 1', [booking.hotelId || metadata.hotelId]);
            ...
            if (booking.hotelId) {
                // Get closest gate (smallest distance)
                const [closestGates] = yield pool.query(`SELECT hg.name_english, hg.gate_number, hgd.distance_meters, hgd.walking_time_minutes
           FROM hotel_gate_distances hgd
           JOIN haram_gates hg ON hgd.gate_id = hg.id
           WHERE hgd.hotel_id = ?
           ORDER BY hgd.distance_meters ASC
           LIMIT 1`, [booking.hotelId]);
                ...
                const [kaabaGates] = yield pool.query(`SELECT hg.name_english, hg.gate_number, hgd.distance_meters, hgd.walking_time_minutes
           FROM hotel_gate_distances hgd
           JOIN haram_gates hg ON hgd.gate_id = hg.id
 ...
 */
exports.userRoutes.get('/me/earnings', auth_middleware_1.authMiddleware, (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = ctx.state.userId;
        const pool = (0, connection_1.getPool)();
        // First, find the agent record for this user
        const [agents] = yield pool.query(`SELECT id FROM agents WHERE user_id = ?`, [userId]);
        ...
        const [hotels] = yield pool.query(`SELECT id, name FROM hotels WHERE agent_id = ?`, [agentId]);
      ...
      ORDER BY b.created_at DESC`, hotelIds);
 ...
 */
exports.userRoutes.get('/me/broker-bookings', auth_middleware_1.authMiddleware, (ctx) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const userId = ctx.state.userId;
        const pool = (0, connection_1.getPool)();
        // First, find the agent record for this user
        const [agents] = yield pool.query(`SELECT id, name, email FROM agents WHERE user_id = ?`, [userId]);
        if (!agents || agents.length === 0) {
