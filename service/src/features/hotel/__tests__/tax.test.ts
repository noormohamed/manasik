...
describe('Tax Service', () => {
  ...
  describe('Get Tax Rate', () => {
    it('should get exact territory match', () => {
      ...
      expect(rate?.territory).toBe('US-NY');
    ...
    it('should fall back to country code', () => {
      ...
      expect(rate).toBeDefined();
      expect(rate?.territory).toBe('US');
      expect(rate?.rate).toBe(0.07);
    });
    ...
    it('should get different state rates', () => {
      ...
      expect(ny?.rate).toBe(0.08);
      expect(ca?.rate).toBe(0.0725);
      expect(tx?.rate).toBe(0.0625);
    });
    ...
    it('should get international rates', () => {
      ...
      expect(fr?.rate).toBe(0.20);
      expect(au?.rate).toBe(0.10);
    });
  });
  ...
  describe('Calculate Tax', () => {
    ...
    it('should throw error for unknown territory', () => {
      expect(() => taxService.calculateTax(1000, 'XX-UNKNOWN')).toThrow(
        'No tax rate found for territory: XX-UNKNOWN'
      );
    });
  ...
  describe('Calculate Booking Price', () => {
    ...
    it('should throw error for unknown territory', () => {
      expect(() =>
        taxService.calculateBookingPrice({
          pricePerNight: 250,
          nights: 3,
          roomCount: 2,
  ...
  describe('Custom Tax Rates', () => {
    it('should use custom tax rates', () => {
      const customRates: TaxRate[] = [
        { territory: 'US', rate: 0.10 },
        { territory: 'CA', rate: 0.15 },
      ];

      const customService = new TaxService(customRates);
      const usRate = customService.getTaxRate('US');
      const caRate = customService.getTaxRate('CA');

      expect(usRate?.rate).toBe(0.10);
      expect(caRate?.rate).toBe(0.15);
    });
