import { BaseBooking, BookingMetadata } from '../../../models/booking/base-booking';
...
export class HotelBooking extends BaseBooking {
  protected metadata!: HotelBookingMetadata;

  validate(): boolean {
    const meta = this.metadata as HotelBookingMetadata;
    ...
    if (totalCapacity < meta.guestCount) {
      throw new Error(
        `Selected rooms (${meta.roomCount} × ${meta.roomOccupancy} capacity) cannot accommodate ${meta.guestCount} guests`
      );
    }
  ...
  getHotelMetadata(): HotelBookingMetadata {
    return this.metadata as HotelBookingMetadata;
  }
  ...
  static createWithTaxCalculation(params: {
    ...
    roomCount: number;
    territory: string;
    metadata: Omit<HotelBookingMetadata, 'taxRate'>;
  }): HotelBooking {
  ...
  static fromJSON(data: any): HotelBooking {
    return HotelBooking.create({
      id: data.id,
