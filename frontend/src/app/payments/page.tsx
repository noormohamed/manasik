...
interface CreditSummary {
  ...
  totalCredits: number;
  pendingCredits: number;      // Bookings not yet checked out
  availableCredits: number;    // Bookings checked out (funds available)
...
export default function PaymentsPage() {
  ...
  const fetchCreditSummary = async () => {
    setLoading(true);
    try {
      // Get exchange rate (USD to GBP)
      let exchangeRate = 0.79; // Default
      try {
      ...
      // Try to fetch host earnings first (bookings at user's hotels)
      try {
      ...
      // If no host earnings, fall back to customer bookings
      if (transactions.length === 0) {
        ...
        bookings.forEach((booking: any) => {
          ...
          // Convert to GBP then to credits (pence)
          let gbpAmount = amount;
          if (currency === 'USD') {
            gbpAmount = amount * exchangeRate;
          } else if (currency === 'EUR') {
            gbpAmount = amount * 0.86; // EUR to GBP
          } else if (currency === 'SAR') {
            gbpAmount = amount * 0.21; // SAR to GBP
          }
          
          const credits = Math.round(gbpAmount * 100);
          
          // Parse check-out date from metadata or booking
          ...
          // Determine if booking is pending or available
    ...
    <>
        ...
        <div className="row">
          <div className="col-xl-8 col-xxl-9">
            <ul className="nav nav-tabs most-popular-tab">
              ...
              <li className="nav-item" role="presentation">
                <button>
                  <Link href="/account/" className={`dropdown-item ${currentRoute === "/account/" ? "active" : ""}`}>
                    Account
                  </Link>
                </button>
              </li>
      ...
      <div className="container pb-5">
        ...
        {summary.recentTransactions.length > 0 && (
          <div className="credit-section transactions-section">
            <div className="section-header">
